...
Use a PAT scoped to the chosen organization with Work Items read/write and Project and Team read permissions. The Branch cleanup and PR cleanup workspaces additionally require Azure Repos Code read/write permissions; repository security and pull-request permissions still apply. Existing project, team and work-item permissions still apply. Additional organization policies may restrict PAT use. Expired or insufficient tokens produce a safe error. Authentication is behind `ITrackerAuthentication` and `ICredentialStore` for future Entra ID support; Entra login is not implemented in this version.
...
- **Sprint workspace:** previous/current/next iteration controls; searchable iteration picker; ID/title/tag search; owner, Active only, Blocked, application, type, tag, area and priority filters. Application tags are separated from ordinary tags, and configured blocked tags are represented by the Blocked filter instead of duplicated in the tag list. Scope is the selected team's configured area paths within the selected iteration.
...
- **Selection:** individual, all visible and shift-range selection. Filtering may retain hidden checkbox state, but every bulk, Smart Fix and AI action is scoped strictly to the currently visible checked rows. Switching iterations clears selection.
...
- **Details:** click a title to open its side panel. View description, criteria, tags, parent, revision, heuristic quality checks and an Azure DevOps link. Daily actions, Smart Fix, Magic Orchestration and final change review also expose the work-item description in collapsible sections so context is available without leaving the workflow. Parent links can open an item outside the loaded sprint.
- **Magic Orchestration:** Home can prepare a Copilot orchestration prompt from open current-sprint work, remaining capacity, estimates, application continuity and known time off. SprintPilot deterministically limits each item to plausible recipient candidates; Copilot may only choose from those candidates and explain why. Recommendations are individually **Accept** / **Ignore**, show before/after load impact, and accepted owner changes still pass through the normal revision-safe Azure DevOps review before anything is written.
...
- **PR cleanup:** browse active pull requests by project and repository, search by PR/branch/creator, and review age, latest source activity, draft state, reviewers, approvals, blocking votes and merge status. Conservative recommendations exclude PRs with approvals, blocking votes, incomplete activity metadata or long-lived source branches. Drafts use the configurable stale threshold; non-drafts require a much longer stale period. Bulk cleanup abandons PRs only after confirmation and re-checks that the PR is still active and its source commit has not changed.
