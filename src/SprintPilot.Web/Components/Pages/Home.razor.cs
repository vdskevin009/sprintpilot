...
public partial class Home {
 ...
 string detailTitle="",detailDescription="",detailAcceptance="",detailOwner="",detailState="",detailTags="",detailEstimate="",repositoryMonitoringError="";
 int sprintIndex,selectionAnchor=-1,templateIndex;
 string search="",ownerSearch="",stateFilter="",typeFilter="",tagFilter="",applicationFilter="",areaFilter="",priorityFilter="",quickView="Team",cleanupFilter="",sort="Order",workspaceMode="Overview";
 ...
 List<RepositoryAttentionSignal> repositoryAttention=[];
 ...
 bool DailyMode=>screen=="workspace"&&workspaceMode=="Daily";
 ...
 DateRange[] DaysOff(string personId){IEnumerable<SprintCapacity> capacities=capacityByIteration.Count>0?capacityByIteration.Values:new[]{sprintCapacity};var azure=capacities.SelectMany(c=>c.TeamDaysOff.Concat(c.Members.FirstOrDefault(m=>m.PersonId==personId)?.DaysOff??[]));var manual=prefs.ManualDaysOff.Where(x=>x.PersonId.Equals(personId,StringComparison.OrdinalIgnoreCase)).Select(x=>new DateRange(x.Start,x.End));return azure.Concat(manual).GroupBy(r=>(r.Start.Date,r.End.Date)).Select(g=>g.First()).OrderBy(r=>r.Start).ToArray();}
 ...
 string HolidayCalendarForPerson(string personId)=>AssignedHolidayCalendars(personId).FirstOrDefault()??"";
 string HolidayCalendarName(string personId)=>HolidayCalendarSummary(personId);
 ...
 HashSet<DateOnly> UnavailableWorkingDates(string personId,Iteration iteration){
  ...
  var calendars=AssignedHolidayCalendars(personId);
  foreach(var holiday in calendarHolidays.Where(h=>calendars.Contains(h.CountryCode,StringComparer.OrdinalIgnoreCase)&&h.End>=first&&h.Start<=last))
 ...
 IReadOnlyList<PersonLane> PeopleLanes(){if(meta is null)return [];IEnumerable<WorkItem> scoped=Visible.Where(w=>w.OwnerId!="");if(DailyMode&&dailyActiveOnly)scoped=scoped.Where(w=>!Quality.Finished(w,meta));var rows=scoped.ToList();return meta.People.Select(p=>new PersonLane(p.Id,p.Name,rows.Where(w=>w.OwnerId==p.Id).OrderBy(WorkGroupRank).ThenBy(w=>w.Order??double.MaxValue).ThenBy(w=>w.Id).ToList())).Where(l=>l.Items.Count>0).ToList();}
 ...
 sealed record RepositoryAttentionSignal(string Key,string Repository,string Label,string Detail);
 async Task LoadRepositoryAttention(CancellationToken ct){repositoryAttention=[];repositoryMonitoringError="";if(!prefs.RepositoryMonitoringEnabled||Tracker.Demo||connection is null)return;repositoryMonitoringLoading=true;try{var repositories=await Tracker.RepositoriesAsync(connection.Project,ct);foreach(var repository in repositories){ct.ThrowIfCancellationRequested();try{var repoBranches=await Tracker.BranchesAsync(connection.Project,repository.Id,ct);foreach(var b in repoBranches){if(b.IsDefault||b.IsLocked||ProtectedBranchConvention(b)||b.LastCommitDate is null)continue;var age=BranchAgeDays(b);if(b.HasCompletedPullRequest&&age>=prefs.BranchCleanupStaleDays)repositoryAttention.Add(new("repo-merged",repository.Name,b.Name,$"Merged branch · inactive {age} days"));else if(!b.HasActivePullRequest&&!b.HasCompletedPullRequest&&age>=Math.Max(180,prefs.BranchCleanupStaleDays*2))repositoryAttention.Add(new("repo-orphan",repository.Name,b.Name,$"No pull request · inactive {age} days"));}var repoPullRequests=await Tracker.PullRequestsAsync(connection.Project,repository.Id,ct);foreach(var p in repoPullRequests.Where(PullRequestRecommended))repositoryAttention.Add(new("repo-pr",repository.Name,$"#{p.Id} · {p.Title}",PullRequestReason(p)));var branchNames=repoBranches.Select(b=>b.Name).ToHashSet(StringComparer.OrdinalIgnoreCase);var prSignals=await Tracker.PullRequestSignalsAsync(connection.Project,repository.Id,ct);foreach(var p in prSignals.Where(p=>p.Status=="abandoned"&&p.CreatedDate!=default&&p.CreatedDate<=DateTimeOffset.UtcNow.AddDays(-prefs.PullRequestCleanupStaleDays)&&branchNames.Contains(p.SourceBranch)&&!ProtectedBranchConvention(p.SourceBranch)))repositoryAttention.Add(new("repo-pr",repository.Name,$"#{p.Id} · {p.Title}",$"Abandoned PR · source branch {p.SourceBranch} still exists"));}catch(OperationCanceledException){throw;}catch(TrackerException e){if(repositoryMonitoringError=="")repositoryMonitoringError=$"{repository.Name}: {e.Message}";}}}catch(OperationCanceledException){throw;}catch(Exception e){repositoryMonitoringError=e is TrackerException?e.Message:"Repository monitoring is unavailable right now.";}finally{repositoryMonitoringLoading=false;}}
 ...
 IEnumerable<(string Id,string Name,int Count)> FilterPeople=>items.GroupBy(w=>w.OwnerId,StringComparer.OrdinalIgnoreCase).Select(g=>(Id:g.Key,Name:PersonName(g.Key),Count:g.Count())).Where(x=>!moreFilters||filterOptionSearch==""||ownerFilters.Contains(x.Id)||x.Name.Contains(filterOptionSearch,StringComparison.OrdinalIgnoreCase)).OrderBy(x=>x.Name=="Unassigned").ThenBy(x=>x.Name);
 ...
 void OpenAttention(string key){if(key=="repo-pr"){_=OpenPullRequestCleanup();return;}if(key is "repo-merged" or "repo-orphan"){_=OpenBranchCleanup();return;}OpenAttentionModal(key);}
 void OpenPerson(string id)=>OpenCurrentPersonModal(id);
 Task OpenSprintPerson(Iteration iteration,string id)=>OpenSprintPersonModal(iteration,id);
...
""".Replace("__CURRENT_SPRINT__",CurrentSprint?.Name??"not available").Replace("__NEXT_SPRINT__",NextSprint?.Name??"not available").Replace("__KNOWN_TAGS__",knownTags).Replace("__MEETING_TITLE__",meetingTitle).Replace("__MEETING_NOTES__",meetingNotes);await JS.InvokeVoidAsync("sprintPilot.copy",prompt);Notify("Copilot meeting prompt copied.");}
 ...
 IReadOnlyList<(string Id,string Name,int Count)> WorkspacePriorityPeople=>items.GroupBy(w=>w.OwnerId,StringComparer.OrdinalIgnoreCase).Select(g=>(Id:g.Key,Name:PersonName(g.Key),Count:g.Count(w=>meta is null||!Quality.Finished(w,meta)))).Where(x=>x.Count>0).OrderBy(x=>x.Name=="Unassigned").ThenBy(x=>x.Name).ToList();
 ...
 string PersonName(string id){
  ...
  return meta?.People.FirstOrDefault(p=>p.Id.Equals(id,StringComparison.OrdinalIgnoreCase))?.Name
   ??items.Concat(previousItems).Concat(planningItems).FirstOrDefault(w=>w.OwnerId.Equals(id,StringComparison.OrdinalIgnoreCase)&&!string.IsNullOrWhiteSpace(w.Owner))?.Owner
   ??$"Unknown owner ({id})";
 }
 ...
 async Task MoveOrder((WorkItem Item,int Delta) move){if(move.Delta==0||!CanOrder(move.Item)||busy.Contains(move.Item.Id))return;sort="Order";descending=false;var visible=Visible.Where(CanOrder).OrderBy(w=>w.Order??double.MaxValue).ThenBy(w=>w.Id).ToList();var visibleIndex=visible.FindIndex(w=>w.Id==move.Item.Id);var targetVisibleIndex=visibleIndex+Math.Sign(move.Delta);if(visibleIndex<0||targetVisibleIndex<0||targetVisibleIndex>=visible.Count)return;var anchor=visible[targetVisibleIndex];var ordered=items.Where(CanOrder).OrderBy(w=>w.Order??double.MaxValue).ThenBy(w=>w.Id).ToList();var sourceIndex=ordered.FindIndex(w=>w.Id==move.Item.Id);if(sourceIndex<0)return;var source=ordered[sourceIndex];ordered.RemoveAt(sourceIndex);var anchorIndex=ordered.FindIndex(w=>w.Id==anchor.Id);if(anchorIndex<0)return;ordered.Insert(move.Delta<0?anchorIndex:anchorIndex+1,source);try{var verified=await MoveSprintItem(ordered,source.Id);if(verified){pendingOrderHighlightId=source.Id;await InvokeAsync(StateHasChanged);}Notify(verified?"Sprint order updated in Azure DevOps.":"Azure DevOps returned a different sprint order. The sprint was refreshed; review the current order.");}catch(Exception e){Error(e);sprintCache.Clear();try{await LoadSprint();}catch{}}}
 ...
 void SetScreen(string target){detail=null;dailyPanelItem=null;if(target=="daily"){screen="workspace";SetWorkspaceMode("Daily");return;}screen=target;if(target=="workspace"){SetWorkspaceMode("Overview");}else if(target=="cleanup"){workspaceMode="Overview";workspaceOwnerMode=false;workspacePriorityOwner=null;}else if(target is "branches" or "pullrequests"){workspaceOwnerMode=false;workspacePriorityOwner=null;}else if(target=="meeting")EnsureMeetingDefaults();}
 void SetWorkspaceMode(string mode){screen="workspace";workspaceMode=mode=="Daily"?"Daily":"Overview";sort="Order";descending=false;workspaceOwnerMode=false;workspacePriorityOwner=null;dailyFocusOwner="";if(workspaceMode=="Daily"){quickView="Team";attentionFilter="";workspaceActiveOnly=true;workspaceBlockedOnly=false;dailyActiveOnly=false;}else workspaceActiveOnly=false;}
 ...
 async Task OpenCleanup(){screen="cleanup";workspaceMode="Overview";ClearFilters();await LoadSprint();}
 ...
 void StartDetailEdit(WorkItem w){dailyPanelItem=null;detail=w;detailTitle=w.Title;detailDescription=ContentText.Plain(w.Description);detailAcceptance=ContentText.Plain(w.Acceptance);detailOwner=OwnerValue(w);detailState=w.State;detailTags=string.Join("; ",w.Tags);detailEstimate=w.Estimate?.ToString(CultureInfo.InvariantCulture)??"";}
 void OpenDetail(WorkItem w)=>StartDetailEdit(w);
 async Task OpenById(int id){try{StartDetailEdit(await Tracker.GetAsync(id,lifetime.Token));}catch(Exception e){Error(e);}}
 void CloseDetail(){if(detailSaving)return;detail=null;}
 void CancelDetailEdits(){if(detail is not null)StartDetailEdit(detail);}
 bool DetailDirty=>detail is not null&&(detailTitle.Trim()!=detail.Title||detailDescription!=ContentText.Plain(detail.Description)||detailAcceptance!=ContentText.Plain(detail.Acceptance)||detailOwner!=OwnerValue(detail)||detailState!=detail.State||detailTags!=string.Join("; ",detail.Tags)||detailEstimate!=(detail.Estimate?.ToString(CultureInfo.InvariantCulture)??""));
 async Task SaveDetail(){if(detail is null||meta is null||detailSaving)return;var original=detail;try{if(string.IsNullOrWhiteSpace(detailTitle)||detailTitle.Trim().Length>255)throw new TrackerException("Enter a title between 1 and 255 characters.");var changes=new List<Change>();var definition=meta.Types.First(t=>t.Name==original.Type);if(detailTitle.Trim()!=original.Title)changes.Add(new(ItemField.Title,detailTitle.Trim()));if(detailOwner!=OwnerValue(original))changes.Add(new(ItemField.Owner,detailOwner));if(detailState!=original.State)changes.Add(new(ItemField.State,detailState));if(detailTags!=string.Join("; ",original.Tags))changes.Add(new(ItemField.Tags,detailTags));if(detailEstimate!=(original.Estimate?.ToString(CultureInfo.InvariantCulture)??"")){object? estimate=null;if(detailEstimate!=""){if(!double.TryParse(detailEstimate,NumberStyles.Float,CultureInfo.InvariantCulture,out var value)||!double.IsFinite(value)||value<0)throw new TrackerException("Estimate must be a non-negative number.");estimate=value;}changes.Add(new(ItemField.Estimate,estimate));}var descriptionChanged=detailDescription!=ContentText.Plain(original.Description);var acceptanceChanged=detailAcceptance!=ContentText.Plain(original.Acceptance);if(definition.Fields.Contains("Microsoft.VSTS.Common.AcceptanceCriteria")){if(descriptionChanged)changes.Add(new(ItemField.Description,ContentText.HtmlEncode(detailDescription)));if(acceptanceChanged)changes.Add(new(ItemField.Acceptance,ContentText.HtmlEncode(detailAcceptance)));}else if(descriptionChanged||acceptanceChanged){var combined=detailDescription;if(!string.IsNullOrWhiteSpace(detailAcceptance))combined+=(combined==""?"":"\n\n")+"ACCEPTANCE CRITERIA:\n"+detailAcceptance;changes.Add(new(ItemField.Description,ContentText.HtmlEncode(combined)));}if(changes.Count==0){Notify($"#{original.Id} has no unsaved changes.");return;}detailSaving=true;busy.Add(original.Id);var saved=await Tracker.UpdateAsync(new(original,changes),lifetime.Token);Replace(saved);StartDetailEdit(saved);sprintCache.Clear();Notify($"#{saved.Id} saved to Azure DevOps.");}catch(Exception e){Error(e);}finally{busy.Remove(original.Id);detailSaving=false;}}
 void Replace(WorkItem w){var i=items.FindIndex(x=>x.Id==w.Id);if(i>=0){var sprintOrder=items[i].Order;items[i]=w with{Order=sprintOrder};}var p=previousItems.FindIndex(x=>x.Id==w.Id);if(p>=0){var sprintOrder=previousItems[p].Order;previousItems[p]=w with{Order=sprintOrder};}var planning=planningItems.FindIndex(x=>x.Id==w.Id);if(planning>=0)planningItems[planning]=w;var modal=homeModalItems.FindIndex(x=>x.Id==w.Id);if(modal>=0)homeModalItems[modal]=w;if(detail?.Id==w.Id)detail=w;}
