...
public sealed class AzureTracker(HttpClient http,ICredentialStore store,ITrackerAuthentication auth,ILogger<AzureTracker> logger):IWorkTracker {
 ...
 public async Task<Metadata> MetadataAsync(bool refresh=false,CancellationToken ct=default){await metadataGate.WaitAsync(ct);try{
  ...
  var types=new List<TypeDefinition>();foreach(var n in Values(await typesTask)){
   ...
   types.Add(new(name,states,fields,estimate));
  }
 ...
 public static string FieldName(ItemField f,TypeDefinition type)=>f switch{ItemField.Title=>"System.Title",ItemField.Owner=>"System.AssignedTo",ItemField.State=>"System.State",ItemField.Iteration=>"System.IterationPath",ItemField.Area=>"System.AreaPath",ItemField.Estimate=>type.EstimateField??throw new TrackerException("This work-item type has no supported estimate field."),ItemField.Priority=>"Microsoft.VSTS.Common.Priority",ItemField.Tags=>"System.Tags",ItemField.Description=>"System.Description",ItemField.Acceptance=>"Microsoft.VSTS.Common.AcceptanceCriteria",_=>throw new TrackerException("Unsupported field.")};
